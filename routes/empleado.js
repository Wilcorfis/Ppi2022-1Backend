...
router.get('/empleadosw', async (req, res) => {
...
fk_id_horario,activo,d.nombre as "fk_id_departamento" from empleado INNER JOIN municipio M
on fk_id_municipio=M.id_municipio inner join departamento d on M.fk_id_departamento=d.id_departamento
inner join tipo_empleado t on t.id_tipo_empleado=fk_id_tipo_empleado inner join genero g on g.id_genero=fk_id_genero where t.nombre='Mesero' or t.nombre='Administrador';
    `);
    return res.status(200).json(rows)
...
router.get('/empleadocorreo', async (req, res) => {

  try {
    const { id } = req.params
    const [rows] = await connection.query(`select correo from empleado ;`);
    return res.status(200).json(rows)
...
router.patch('/empleadoclave/:id', async (req, res) => {
  try {
    const { id } = req.params
